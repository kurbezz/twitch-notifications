use axum::{body::Body, http::{Request, Response, HeaderValue}, middleware::Next};
...
pub async fn csp_middleware<B>(req: Request<B>, next: Next<B>) -> Response {
    ...
    if res.headers().get("content-security-policy").is_none() {
        if let Ok(val) = HeaderValue::from_static(CSP) {
            res.headers_mut().insert("content-security-policy", val);
        }
    }

    // Also add Referrer-Policy and X-Content-Type-Options for extra security
    if res.headers().get("referrer-policy").is_none() {
        if let Ok(val) = HeaderValue::from_static("no-referrer") {
            res.headers_mut().insert("referrer-policy", val);
        }
    }
    if res.headers().get("x-content-type-options").is_none() {
        if let Ok(val) = HeaderValue::from_static("nosniff") {
